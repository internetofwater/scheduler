...
from .lib.dagster import filter_partitions, slack_error_fn
from .lib.env import (
    RUNNING_AS_TEST_OR_DEV,
    strict_env,
)
from .assets import configs, exports, pipeline

"""
This file defines all of the core dagster functionality
for starting, monitoring, or initializing the 
pipeline for Geoconnex and the associated jobs.
"""
...
harvest_job = define_asset_job(
    ...
    - AssetSelection.groups("exports")
    - AssetSelection.groups("configs"),
)
...
config_job = define_asset_job(
    "configs",
    description="update all configs needed before starting the pipeline",
    selection=AssetSelection.groups("configs"),
)


# expose all the code needed for our dagster repo
definitions = Definitions(
    assets=load_assets_from_modules([pipeline, exports, configs]),
    asset_checks=load_asset_checks_from_modules([pipeline, exports, configs]),
    jobs=[harvest_job, export_job, config_job],
    sensors=[
    ...
    # Commented out but can uncomment if we want to send other arbitrary slack msgs
    # resources={
...
def crawl_entire_graph_schedule(context: ScheduleEvaluationContext):
    get_dagster_logger().info(
        "Schedule triggered; Deleting old partition status before new crawl"
    )
    filter_partitions(context.instance, "sources_partitions_def", keys_to_keep=set())

    result = definitions.get_job_def("config").execute_in_process(
...
with DagsterInstance.get() as instance:
    assert (
        definitions.get_job_def("configs").execute_in_process(instance=instance).success
    ), "Failed to initialize the config at startup"


definitions = Definitions.merge(
    definitions, Definitions(schedules=[crawl_entire_graph_schedule])
)
